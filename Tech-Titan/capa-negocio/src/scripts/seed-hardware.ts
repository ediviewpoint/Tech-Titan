...
const HARDWARE_CATALOG: HardwareComponent[] = [
...

  {
    ...
    metadata: {
      ...
      ram_generation: RamGeneration.DDR5,
    },
  },
  {
  ...
  },
  {
    ...
    category: ComponentCategory.RAM,
    metadata: {
      ram_generation: RamGeneration.DDR4,
    },
  },
...
async function upsertComponent(client: Client, component: HardwareComponent): Promise<void> {
  // UPSERT: si el nombre ya existe, actualiza metadata (permite re-seed seguro)
  const result: QueryResult<{ id: string; action: string }> = await client.query(
    ...
    [component.name, component.category, JSON.stringify(component.metadata)]
  );
...
async function main(): Promise<void> {
  ...
  try {
    ...
    // ── Resumen post-seed ──────────────────────────────────────────────────
    const counts: QueryResult<{ category: string; total: string }> = await client.query(
    ...
    const total: QueryResult<{ total: string }> = await client.query(
      "SELECT COUNT(*)::text AS total FROM hardware_components"
    );
