...
async function main() {
  ...
  try {
    ...
    const existing = await client.query("SELECT id, email FROM public.user WHERE email = $1", [ADMIN_EMAIL]);
    ...
    await client.query(`INSERT INTO public.user
       ...
       VALUES ($1, $2, $3, $4, $5, 'admin', NOW(), NOW())`, [id, ADMIN_EMAIL, passwordHash, ADMIN_FIRSTNAME, ADMIN_LASTNAME]);
    console.log("\n════════════════════════════════════════════");
