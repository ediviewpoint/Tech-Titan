...
exports.default = exports.SocketCompatibilityRule = exports.RamCompatibilityRule = exports.PowerConsumptionRule = exports.PCBuildValidator = void 0;
exports.validatePCBuild = validatePCBuild;
...
class SocketCompatibilityRule {
  constructor() {
    this.ruleName = "SocketCompatibility";
  }
  validate(items) {
    const cpu = items.find(i => i.category === _hardware.ComponentCategory.CPU);
    const mb = items.find(i => i.category === _hardware.ComponentCategory.MOTHERBOARD);
    ...
    if (!cpuSocket || !mbSocket) {
      ...
      return {
        errors: [],
        warnings: [`No se pudo verificar compatibilidad de socket: faltan datos en "${missing}".`]
      };
    }
    if (cpuSocket !== mbSocket) {
      return {
...
exports.SocketCompatibilityRule = SocketCompatibilityRule;
class RamCompatibilityRule {
  constructor() {
    this.ruleName = "RamCompatibility";
  }
  validate(items) {
    const rams = items.filter(i => i.category === _hardware.ComponentCategory.RAM);
    const mb = items.find(i => i.category === _hardware.ComponentCategory.MOTHERBOARD);
    ...
    if (!mbRamGen) {
      return {
        errors: [],
        warnings: [`"${mb.name}" no tiene generacion de RAM definida en sus metadatos. Actualiza el seed con ram_generation para habilitar esta validacion.`]
      };
    }
    const errors = [];
    for (const ram of rams) {
      ...
      if (!ramGen) {
        errors.push(`"${ram.name}" no tiene generacion de RAM en sus metadatos.`);
        continue;
      }
      if (ramGen !== mbRamGen) {
...
exports.RamCompatibilityRule = RamCompatibilityRule;
const PSU_HIGH_LOAD_THRESHOLD = 0.8;
class PowerConsumptionRule {
  constructor() {
    this.ruleName = "PowerConsumption";
  }
  validate(items) {
    const psu = items.find(i => i.category === _hardware.ComponentCategory.PSU);
    const totalTdp = items.reduce((acc, item) => acc + (item.metadata.tdp_watts ?? 0), 0);
    if (totalTdp === 0) return EMPTY;
    if (!psu) {
      return {
        errors: [],
        warnings: [`Consumo estimado de ${totalTdp}W detectado pero no hay Fuente de Poder (PSU) en el carrito.`]
      };
    }
    ...
    if (!psuWattage) {
      return {
        errors: [],
        warnings: [`"${psu.name}" no tiene wattage definido. Agrega wattage_watts a sus metadatos para verificar consumo.`]
      };
    }
    ...
    if (totalTdp / psuWattage > PSU_HIGH_LOAD_THRESHOLD) {
      return {
        errors: [],
        warnings: [`Alta carga en PSU: el sistema consume ~${totalTdp}W (${loadPercent}% de ${psuWattage}W). Se recomienda una PSU de mayor capacidad para garantizar estabilidad.`]
      };
    }
    return EMPTY;
  }
...
exports.PowerConsumptionRule = PowerConsumptionRule;
class PCBuildValidator {
  constructor(rules) {
    this.rules = void 0;
    this.rules = rules;
  }
...
// ─── Public API ───────────────────────────────────────────────────────────────
exports.PCBuildValidator = PCBuildValidator;
const defaultValidator = new PCBuildValidator([new SocketCompatibilityRule(), new RamCompatibilityRule(), new PowerConsumptionRule()]);
 ...
 */
function validatePCBuild(items) {
...
var _default = exports.default = CompatibilityService;